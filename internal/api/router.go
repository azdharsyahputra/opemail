...
func NewRouter(deps RouterDependencies) http.Handler {
	...
	domH := handler.NewDomainHandler(deps.DomainService, deps.DKIMService, deps.TLSService)
	mbH := handler.NewMailboxHandler(deps.MailboxService)
	...
	r.Group(func(apiGroup chi.Router) {
		...
		apiGroup.Route("/api/v1/domains", func(dr chi.Router) {
			...
			dr.With(middleware.RequireRole("admin", "operator")).Post("/", domH.Create)
			dr.With(middleware.RequireRole("admin", "operator", "auditor")).Get("/{domain}", domH.Get)
			dr.With(middleware.RequireRole("admin", "operator")).Delete("/{domain}", domH.Delete)
			...
			dr.With(middleware.RequireRole("admin", "operator")).Post("/{domain}/dkim/{selector}/revoke", dkimH.Revoke)

			// TLS
			dr.With(middleware.RequireRole("admin", "operator", "auditor")).Get("/{domain}/tls", tlsH.Get)
			dr.With(middleware.RequireRole("admin", "operator")).Post("/{domain}/tls", tlsH.Install)
		...
		apiGroup.Route("/api/v1/mailboxes", func(mr chi.Router) {
			...
			mr.With(middleware.RequireRole("admin", "operator", "auditor", "user")).Get("/{email}", mbH.Get)
			mr.With(middleware.RequireRole("admin", "operator")).Delete("/{email}", mbH.Delete)
			...
			mr.With(middleware.RequireRole("admin", "operator", "user")).Post("/{email}/password", mbH.SetPassword)

			// Aliases
			mr.With(middleware.RequireRole("admin", "operator", "auditor", "user")).Get("/{email}/aliases", aliasH.List)
			...
			// Quota
			mr.With(middleware.RequireRole("admin", "operator", "auditor", "user")).Get("/{email}/quota", quotaH.Get)
		...
		apiGroup.Route("/api/v1/ldap", func(lr chi.Router) {
			...
			lr.With(middleware.RequireRole("admin", "operator")).Post("/sync", identH.LDAPSync)
		})
		...
		apiGroup.Route("/api/v1/queue", func(qr chi.Router) {
			...
			qr.With(middleware.RequireRole("admin", "operator")).Post("/flush", qH.Flush)
		})
